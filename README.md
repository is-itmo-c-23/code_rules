...
```
### 6) Если вы параметризуете какой-то тип(Это когда вот так например std::vector<std::vector>, здесь параметризуем вектор интом), то перед `<` пробел не нужен
